...
class ConvSinkhorn:
    def __init__(self, tau, order, method, graph=None, lap=None):
        ...
        self.heat_filter = HeatFilter(tau = tau, order = order, method= method,**kwargs_filter)

    def __call__(self, m_0, m_1, stopThr=1e-4, max_iter=1e3, verbose=False, return_vw=False, stopping_crit=False, stopping_fn=lambda x: False):
        eps = 1e-8
        ...
        for i in range(1, int(max_iter) + 1):
            ...
            if np.any(np.isnan(v)) or np.any(np.isnan(w)) or np.any(np.isinf(v)) or np.any(np.isinf(w)):
                v = v_prev
                w = w_prev
                break
            if i % 100 == 0:
                ...
                if np.sum(np.abs(v - v_prev)) < stopThr:
                    ...
                    break
            if stopping_crit:
                kl = np.sum(4 * self.tau * a * (m_0 * np.log(v + eps) + m_1 * np.log(w + eps)))
                if stopping_fn(kl):
                    print("converged at iteration %d" % i)
                    break
        kl = np.sum(4 * self.tau * a * (m_0 * np.log(v + eps) + m_1 * np.log(w + eps)))
